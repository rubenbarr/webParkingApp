...
export default function Page() {
  ...
  async function getKioscoData(kioscoId: string) {
    ...
    if (req) {
      ...
      if(!req.state) {
        handleToast('error', 'Hubo un error intentando conseguir la información del kiosco, consulte a administración')
  ...
  async function getKioscoMoneyDataReq(kioscoId: string) {
    setLoadingGlobal(true);
    const req = await getKioscoMoneyData(token as string, kioscoId) as Response;
    if (req) {
      setLoadingGlobal(false);
      if(!req.state) {
        handleToast('error', 'Hubo un error intentando conseguir la información financiera del kiosco, consulte a administración')
      }
    ...
    return (
      <div className="entity-content">
        <div className="main-entity-content">
          <label> <b>{"Titulo: "}</b>{info.title}</label>
          <label> <b>{"Contacto: "}</b>{info.contactName}</label>
          <label> <b>{"Teléfono de contacto: "}</b>{info.contactPhone}</label>
          <label> <b>{"Ubicación: "}</b>{location}</label>
          <label> <b>{"Número de serie: "}</b>{info.serialNumber}</label>
        </div>
        <div className="status-entity-content">
          <div className="left-data">
          <b>{"Estatus: "}<label className={ cn({'offline': info.online === 'Offline'})}>{info.online}</label></b>
          <b>{"Activo: "}<label className={cn({'inactive': info.active === "Inactivo", "active": info.active === 'Activado'})}>{info.active}</label></b>
          </div>

          { info.active === "Inactivo" && 
          <button className="primary-button" onClick={() => setDisplayActKey(true)}>Activar Kioscos</button>
          }
        </div>
      </div>
    )
  ...
  const generateActivationKey = () => {
    return(
    kioscoData &&  kioscoData.active && displayActivationKey &&  
    <div className="activation-key-container">
      <div className="header-activation-container">
        <div className="main-header-title">
          <h1>Codigo de activación</h1>
          <button onClick={() => setDisplayActKey(false)} className="close-button">Cerrar</button>
        </div>
        <p>Para activar kiosco escanee el codigo QR en el lector del kiosco </p>
      </div>
        <QRCodeSVG value={kioscoData.activationKey} width={300} height={300} />
      </div>
    )
  }
  ...
  return (
    <div className="kiosco-info-container">
      ...
      {kioscoData && <button className="primary-button" onClick={()=> {getKioscoData(kioscoId as string); setDisplayActKey(false);}}>Refrescar Datos</button>}
      </div>
      { kioscoData != null &&  
      <>
      {kioscoDataContent()}
