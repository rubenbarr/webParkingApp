...
export async function getLocations(token:string, page:number, limit:number ) {
    try {
        const req = await fetchWithTimeout(`${LOCALHOST}/api/locations_route/getLocations?page=${page}&limit=${limit}`, 
            {
                method:GET,
...
export async function createLocation(token: string, data: object) {
    try {
        const req = await fetchWithTimeout(`${LOCALHOST}/api/locations_route/createLocation`, {
            method: POST,    
            headers: headers(token),
...
export async function getLocationById(token: string, id:string) {
    try {
        const req = await fetchWithTimeout(`${LOCALHOST}/api/locations_route/getLocation/${id}`, {
            method: GET,    
            headers: headers(token),
        });
        return req;
